...
def calculate_layers_and_leaves(ontology_roots, reversed_graph):
    """
    Calculate the layer (distance from root) for each node in the reversed ontology graph and identify leaf nodes.

    Args:
        ontology_roots (list): List of root ontology terms.
        reversed_graph (networkx.DiGraph): The reversed directed ontology graph.

    Returns:
        tuple:
            max_layer_by_root (dict): Maximum layer for each root term.
            all_leaves (set): Set of all leaf nodes in the ontology.
            subgraph_by_root (dict): Subgraph (reachable nodes) for each root.
            updated_graph (networkx.DiGraph): Updated graph with layer information.
    """
    max_layer_by_root = {}
    all_leaves = set()
    ontology_subgraph = {}
    for root in ontology_roots:
        subgraph = []
        max_layer_by_root[root] = 0
        queue = deque([root])
        visited = {root}

        reversed_graph.nodes[root]["Layer_of"+root] = 0
        subgraph.append(root)

        while queue:
            current = queue.popleft()
            neighbors = reversed_graph.neighbors(current)

            for neighbor in neighbors:
                if neighbor not in visited:
                    reversed_graph.nodes[neighbor]["Layer_of"+root] = (
                        reversed_graph.nodes[current]["Layer_of"+root] + 1
                    )
                    subgraph.append(neighbor)
                    queue.append(neighbor)
                    visited.add(neighbor)
                    if max_layer_by_root[root] < reversed_graph.nodes[neighbor]["Layer_of"+root]:
                        max_layer_by_root[root] = reversed_graph.nodes[neighbor]["Layer_of"+root]

            if not any(reversed_graph.neighbors(current)):
                all_leaves.add(current)

        ontology_subgraph[root] = subgraph

    return max_layer_by_root, all_leaves, ontology_subgraph, reversed_graph


def get_leaves_by_ontology(ontology_roots, reversed_graph):
    """
    Retrieve the leaf nodes for each root ontology and calculate layer information.
    ...
    Returns:
        tuple:
            max_layer_by_root (dict): Maximum layer for each root term.
            leaves_by_root (dict): Dictionary of leaf nodes for each root term.
            subgraph_by_root (dict): Subgraph (reachable nodes) for each root.
            updated_graph (networkx.DiGraph): Updated graph with layer information.
    """
    max_layer_by_root, all_leaves, ontology_subgraph, reversed_graph = calculate_layers_and_leaves(
        ontology_roots, reversed_graph)

    leaves = {root: list(all_leaves) for root in ontology_roots}
    return max_layer_by_root, leaves, ontology_subgraph, reversed_graph


def print_ontology_info(max_layers, leaves, subgraphs):
    """
    Print the calculated ontology information.

    Args:
        max_layers (dict): Maximum layer information by root ontology.
        leaves (dict): Leaf nodes for each root ontology.
        subgraphs (dict): Subgraphs for each root ontology.
    """
    print("Maximum layer by root ontology:", max_layers)
    print("Leaf nodes by ontology:")
    for root, leaf_nodes in leaves.items():
        print(f"  {root}: {leaf_nodes}")
    print("Subgraph nodes by ontology:")
    for root, subgraph in subgraphs.items():
        print(f"  {root}: {subgraph}")

if __name__ == "__main__":
    GOontologySlim = obonet.read_obo("/home/pospim/Desktop/work/GOLizard/golizard/golizz/bash_app/meta/goslim_plant.obo")
    ...
    GOontologySlim_reverse = GOontologySlim.reverse()

    most_distance, leaves, subgraph_ontology, GOontologySlim_reverse = get_leaves_by_ontology(pivot, GOontologySlim_reverse)
    print_ontology_info(most_distance, leaves, subgraph_ontology, GOontologySlim_reverse)
